      ...
      <section class="legal-banner overflow-hidden bg-legalbg bg-cover" v-if="showAllLegal">
        <div class="legal-banner-content w-full pt-40 pb-20">
          <div class="w-full px-6 mx-auto mesm:px-12 memd:px-16 melg:max-w-[1200px] melg:px-24 flex flex-col items-start justify-center gap-12">
            <h1 class="w-full text-white text-3xl font-bold mesm:text-4xl melg:text-5xl text-center">LEGAL INFOMATION</h1>
            <div class="w-full desc text-base text-gray-700">
              <p class="w-full text-gray-300 text-base md:text-xl py-1 text-center">To fully understand the implications of trading with ABC Private Limited, please read our legal documents.</p>
            </div>
            <!-- <div>
      ...
      <section class="legal-banner overflow-hidden bg-cover py-20">
        <div class="w-full px-6 mx-auto mesm:px-12 memd:px-16 melg:max-w-[1200px] melg:px-24 flex flex-col items-start justify-center gap-12">
          <div class="legal-list" v-if="showAllLegal">
            <div class="legal-item flex flex-row justify-start items-center gap-4 py-4 hover:text-blue-600 hover:underline hover:cursor-pointer" v-for="(item, i) in legalList" :key="i" @click="showLegalDetail(i)">
              <span class="ico"
                ><svg t="1739791612232" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="14141" width="24" height="24">
                  <path
