<template>
  <div class="category-wrapper h-full">
    <div ref="scrollContainer" class="category-container h-full overflow-auto">
      <!-- Place your trades on best conditions -->
      <div class="w-full pt-28 pb-20 mesm:pt-40">
        <div class="w-full px-6 mx-auto mesm:px-12 memd:px-16 melg:max-w-[1200px] melg:px-24">
          ...
          <div class="back" @click="goBack">
            <svg viewBox="0 0 24 24" role="presentation" focusable="false" aria-hidden="true" data-role="icon">
              <path fill="currentColor" fill-rule="evenodd" d="M14.707 7.293a1 1 0 00-1.414 0l-4 4a1 1 0 000 1.414l4 4a1 1 0 001.414-1.414L11.414 12l3.293-3.293a1 1 0 000-1.414z" clip-rule="evenodd"></path>
            </svg>
          ...
          <div class="bodies">
            <!-- aside -->
            <div class="aside" v-if="windowSize.width > 670 || (windowSize.width < 671 && !route.params.url_alias)">
              <div class="aside-container">
                <div v-for="(item, i) in specificCateList" :key="i" class="aside-block-item">
                  <div class="block-header" @click="handleHeaderClick(item)" :class="{ expand: item.isExpand }">
                    <div class="left">
                      <div class="a-title">{{ item.title }}</div>
                      <div class="topics">{{ item.playlists.length }} topics</div>
                    </div>
                    <div class="right">
                      <svg class="SvgIcon-module-host-3SE _20G6J9d" viewBox="0 0 24 24" role="presentation" focusable="false" aria-hidden="true" data-role="icon">
                        <path fill="currentColor" fill-rule="evenodd" d="M16.707 14.707a1 1 0 000-1.414l-4-4a1 1 0 00-1.414 0l-4 4a1 1 0 101.414 1.414L12 11.414l3.293 3.293a1 1 0 001.414 0z" clip-rule="evenodd"></path>
                      </svg>
                    </div>
                  </div>
                  <div class="block-bodies" :class="{ expand: item.isExpand }">
                    <div v-for="(ele, k) in item.playlists" :key="k" class="bodies-item" :class="{ active: ele.id === activePlaylistId }" @click="handlePlaylistItem(ele)">
                      <svg viewBox="0 0 24 24" fill="currentColor" data-role="icon" class="_3qcjcvF">
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M2 6C2 5.44772 2.44772 5 3 5H21C21.5523 5 22 5.44772 22 6C22 6.55228 21.5523 7 21 7H3C2.44772 7 2 6.55228 2 6ZM2 12C2 11.4477 2.44772 11 3 11H21C21.5523 11 22 11.4477 22 12C22 12.5523 21.5523 13 21 13H3C2.44772 13 2 12.5523 2 12ZM2 18C2 17.4477 2.44772 17 3 17H15C15.5523 17 16 17.4477 16 18C16 18.5523 15.5523 19 15 19H3C2.44772 19 2 18.5523 2 18Z"></path>
                      </svg>
                      <span>{{ ele.title }}</span>
                    </div>
                  </div>
            ...
            <div class="article" v-if="windowSize.width > 670 || (windowSize.width < 671 && route.params.url_alias)">
...
import { useDomScroll } from '@/hooks/useDomScroll.ts'
